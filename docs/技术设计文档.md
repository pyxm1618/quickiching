...
| Creem | Provisional | 商户、产品和退款政策审核通过；否则切换支付适配实现 |
...
| 支付 | Creem | Provisional | Checkout、签名 Webhook、退款和争议事件 |
...
1. 读取原始请求体；
2. 使用官方 SDK 或 HMAC-SHA256 验证 `creem-signature`，手写比较必须常量时间；
3. 校验事件 Schema、环境和类型；
...
CSP 只开放 Better Auth、Creem、Turnstile 和批准分析域名所需来源；禁止宽泛 `unsafe-eval`。
...
- `/i-ching-coin` 与 `/three-coin-method` 使用不同搜索意图和正文；
...
- local；
- preview：独立数据库分支、Creem Test Mode、测试 Turnstile、独立 Auth URL、邮件白名单；
...
- 验证 Creem Checkout、Webhook、退款和争议；
- 输出可执行金标准，不产生生产 Mock。
...
- Creem Checkout、Webhook Inbox、退款和争议；
...
- Creem Checkout：<https://docs.creem.io/api-reference/endpoint/create-checkout>
...
11. 使用 Creem 作为暂定支付商，同时保留最薄 PaymentProvider 边界应对审批失败。
