...
export function decodeVersionedKeyValue(value: string): Buffer {
  if (value.startsWith("base64:")) {
    const decoded = decodeBase64Strict(value.slice("base64:".length));
    if (decoded) return decoded;
    throw new Error("KEY_MATERIAL_INVALID");
  }
  if (value.startsWith("hex:")) {
    const decoded = decodeHexStrict(value.slice("hex:".length));
    if (decoded) return decoded;
    throw new Error("KEY_MATERIAL_INVALID");
  }
