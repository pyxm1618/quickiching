...
import { withPostgresAdvisoryTransactionLock } from "@/server/db/advisory-lock";
...
export async function dispatchGenerationOutbox(limit = 25): Promise<{
  ...
  const protectedDispatch = await withPostgresAdvisoryTransactionLock(
    runtime.sql,
  ...
  return protectedDispatch.acquired
    ? protectedDispatch.value
