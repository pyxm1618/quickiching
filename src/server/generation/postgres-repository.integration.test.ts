...
describe("CP3 PostgreSQL Preview repository", () => {
  ...
  it("persists a reviewed Preview atomically and reuses the same idempotency job", async () => {
    ...
    const storedJob = await repository.getJobStatus(castingId, "preview-idempotency-1");
    ...
    await sql`
      insert into generation_jobs (
        ...
        'rollback-snapshot', ${new Date(now.getTime() + 30_000).toISOString()}, 1,
        'test', ${rollbackLease}, ${new Date(now.getTime() + 30_000).toISOString()},
        ${now.toISOString()}, ${now.toISOString()}
      )
    `;
    await sql`
