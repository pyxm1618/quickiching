    ...
    <section className="rounded-2xl border border-[var(--line-strong)] bg-[var(--paper-raised)] p-5 sm:p-8" aria-labelledby="mei-hua-tool-title">
      ...
      <div className="mt-6 max-w-xl">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row">
          <input id="mei-hua-timezone" value={timeZone} onChange={(event) => setTimeZone(event.target.value)} disabled={Boolean(cast)} spellCheck={false} className="min-h-11 min-w-0 flex-1 rounded-md border border-[var(--line-strong)] bg-[var(--paper)] px-3 py-2 font-mono text-sm outline-none focus:ring-2 focus:ring-[var(--jade)] disabled:opacity-70" aria-describedby="mei-hua-timezone-help mei-hua-error" />
      ...
      {calculation ? (
        <div className="mt-6 min-w-0 rounded-xl border border-[var(--line)] bg-[var(--paper)] p-5">
          ...
          <dl className="mt-4 grid min-w-0 gap-x-6 gap-y-2 text-sm text-[var(--ink-2)] sm:grid-cols-2">
            <div className="flex min-w-0 justify-between gap-4"><dt className="shrink-0">Timezone</dt><dd className="min-w-0 break-all text-right font-mono">{String(calculation.ianaTimeZone)}</dd></div>
            <div className="flex min-w-0 justify-between gap-4"><dt className="shrink-0">Formula date</dt><dd className="text-right font-mono">{String(calculation.year)}-{String(calculation.month).padStart(2, "0")}-{String(calculation.day).padStart(2, "0")}</dd></div>
            <div className="flex min-w-0 justify-between gap-4"><dt className="shrink-0">Local hour</dt><dd className="text-right font-mono">{String(calculation.hour).padStart(2, "0")}:xx</dd></div>
            <div className="flex min-w-0 justify-between gap-4"><dt className="shrink-0">Year / hour branch</dt><dd className="text-right font-mono">{String(calculation.yearBranchNumber)} / {String(calculation.hourBranch)}</dd></div>
            <div className="flex min-w-0 justify-between gap-4"><dt className="shrink-0">Upper / lower trigram no.</dt><dd className="text-right font-mono">{String(calculation.upperTrigramNumber)} / {String(calculation.lowerTrigramNumber)}</dd></div>
            <div className="flex min-w-0 justify-between gap-4"><dt className="shrink-0">Changing line</dt><dd className="text-right font-mono">{String(calculation.movingLinePosition)}</dd></div>
          </dl>
        </div>
      ) : null}
