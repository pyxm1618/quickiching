...
export default async function ChinesePricingPage(props: { searchParams?: Promise<{ returnUrl?: string; preview?: string }> }) {
  ...
  if (!pricing.enabled) {
    return <section className="mx-auto max-w-3xl px-4 py-12 sm:py-16"><p className="font-mono text-[11px] tracking-[0.14em] text-[var(--bronze)]">商业功能 · 当前未启用</p><h1 className="mt-3 font-display text-4xl font-medium tracking-tight">个性化深度解读当前未开放购买</h1><p className="mt-5 text-lg leading-8 text-[var(--ink-2)]">目前免费的四种起卦方法与完整通用卦象解读仍可使用，包括三枚铜钱、蓍草、梅花易数和手动起卦。免费解读绝不调用 AI。当前部署没有开放正式支付、解读次数购买或付费深度解读。</p><div className="mt-8 rounded-xl border border-[var(--line)] bg-[var(--paper-raised)] p-6 text-sm leading-7 text-[var(--ink-2)]"><strong className="text-[var(--ink)]">付费商品边界：</strong>如果深度解读开放，它将结合起卦时锁定的核心问题、你提供的现实背景、解释目标、本次确切卦象、动爻、之卦和可核对的易经材料，解释卦象与你具体处境的关系；当前只支持三枚铜钱法。购买条款仅在支付启用时展示。</div><p className="mt-8 text-sm"><Link href="/zh" className="font-semibold text-[var(--jade)] hover:underline">返回免费中文起卦 →</Link></p></section>;
  }
  return <section className="mx-auto max-w-5xl px-4 py-12 sm:py-16"><p className="font-mono text-[11px] tracking-[0.14em] text-[var(--bronze)]">个性化深度解读</p><h1 className="mt-3 font-display text-4xl font-medium tracking-tight">选择深度解读次数包</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-[var(--ink-2)]">付费深度解读回答“这次卦象对我的具体处境意味着什么”。它会结合起卦前锁定的核心问题、你补充的现实背景和解释目标、本次准确的三枚铜钱卦象与动爻、之卦和 Quick I Ching 提供的易经材料，并展示可核对的解释依据。免费解读仍完整解释卦象本身，且绝不调用 AI。</p><p className="mt-4 rounded-xl border border-[var(--line)] bg-[var(--paper-raised)] p-5 text-sm leading-7 text-[var(--ink-2)]">当前付费支持范围：<strong className="text-[var(--ink)]">仅三枚铜钱起卦</strong>。其他方法仍可免费完整解读，但不能购买深度解读。</p>
